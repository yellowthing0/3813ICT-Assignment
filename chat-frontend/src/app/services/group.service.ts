...
export class GroupService {
  ...
  // Fetch channels in a specific group
  fetchChannels(group: string): Observable<any> {
    return this.http.get(`${this.apiUrl}/groups/${group}/channels`).pipe(
      catchError((error) => {
        console.error('Error fetching channels for group', error);
        return throwError(
          () => new Error('Error fetching channels for group.')
        );
      })
    );
  }
  ...
  createGroup(groupName: string, username: string): Observable<any> {
    return this.http
      .post(`${this.apiUrl}/groups`, { groupName, username })
      .pipe(
        catchError((error) => {
  ...
  deleteGroup(groupName: string, username: string): Observable<any> {
    return this.http
      .delete(`${this.apiUrl}/groups/${groupName}?username=${username}`)
      .pipe(
        catchError((error) => {
  ...
  inviteUserToGroup(
    ...
    return this.http
      .post(`${this.apiUrl}/groups/${groupName}/invite`, {
        invitedUsername,
        invitingUsername,
      })
      .pipe(
  ...
  removeUserFromGroup(
    ...
    return this.http
      .post(`${this.apiUrl}/groups/${groupName}/remove`, {
        removedUsername,
        removingUsername,
      })
      .pipe(
